...
static void load_modules(const multiboot_info* mbi){
  if (mbi->mods_count > 0) {
    ...
    for (int i = 0; i < (initrd_size/4072)+1; ++i) {
      unsigned int adress = initrd_location + i * 4096;
...
/* le main du noyau */
extern "C" void kernal_main(const multiboot_info* mbi) {
  io.clear();
  io.print("%s - %s -- %s %s \n",
                  KERNEL_NAME,
                  KERNEL_VERSION,
