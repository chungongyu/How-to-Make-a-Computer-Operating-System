...
idt_desc kidt[IDTSIZE];     /* IDT table */
 ...
 */
void gdt_desc_init(struct gdt_desc* desc, u32 base, u32 limite, u8 acces, u8 other) {
...
void gdt_init() {
  ...
  gdt_desc_init(&kgdt[0], 0x0, 0x0,     0x0,  0x0);
  gdt_desc_init(&kgdt[1], 0x0, 0xFFFFF, 0x9B, 0x0D);  /* code */
  gdt_desc_init(&kgdt[2], 0x0, 0xFFFFF, 0x93, 0x0D);  /* data */
  gdt_desc_init(&kgdt[3], 0x0, 0x0,     0x97, 0x0D);  /* stack */

  gdt_desc_init(&kgdt[4], 0x0, 0xFFFFF, 0xFF, 0x0D);  /* ucode */
  gdt_desc_init(&kgdt[5], 0x0, 0xFFFFF, 0xF3, 0x0D);  /* udata */
  gdt_desc_init(&kgdt[6], 0x0, 0x0,     0xF7, 0x0D);  /* ustack */

  gdt_desc_init(&kgdt[7], (u32) & default_tss,
                               0x67,    0xE9, 0x00);  /* descripteur de tss */
  ...
  memcpy((char *)kgdtr.base, (const char *)kgdt, kgdtr.limite);
...
void idt_desc_init(struct idt_desc* desc, u16 select, u32 offset, u16 type) {
...
void idt_init() {
  ...
  for (int i = 0; i < IDTSIZE; i++)
    idt_desc_init(&kidt[i], 0x08, (u32)_asm_schedule, INTGATE); // 

  /* Vectors  0 -> 31 are for exceptions */
  idt_desc_init(&kidt[13], 0x08, (u32) _asm_exc_GP, INTGATE);    /* #GP */
  idt_desc_init(&kidt[14], 0x08, (u32) _asm_exc_PF, INTGATE);    /* #PF */
  
  idt_desc_init(&kidt[32], 0x08, (u32) _asm_schedule, INTGATE);
  idt_desc_init(&kidt[33], 0x08, (u32) _asm_int_1, INTGATE);
  
  idt_desc_init(&kidt[48], 0x08, (u32) _asm_syscalls, TRAPGATE);
  idt_desc_init(&kidt[128], 0x08, (u32) _asm_syscalls, TRAPGATE); //48
  
  kidtr.limite = IDTSIZE * 8;
  kidtr.base = IDTBASE;
  
  /* Copy the IDT to the memory */
  memcpy((char *)kidtr.base, (const char *)kidt, kidtr.limite);
...
void schedule() {
  ...
  //io.print("schedule %s ",pcurrent->name());
