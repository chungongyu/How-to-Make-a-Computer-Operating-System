...
void Architecture::init_process() {
  first_proc = new Process("kernel");
  first_proc->status(ZOMBIE);
  first_proc->addFile(fsm.path("/dev/tty"),0);
  first_proc->addFile(fsm.path("/dev/tty"),0);
  first_proc->addFile(fsm.path("/dev/tty"),0);
...
void Architecture::add_process(Process* p) {
  p->setPNext(plist);
  plist=p;
}
...
int Architecture::fork(process_st* info,process_st* father) {
  ...
  info->pd = page_directory_copy(father->pd);
}

/* Initialise a new process */
int Architecture::create_process(process_st* info, char* file, int argc, char** argv) {
  page *kstack;
  process_st *previous;
  process_st *current;
  ...
  if (e_entry == 0) {  
    ...
    page_directory_destroy(info->pd);
    return -1;
  }
...
void Architecture::destroy_process(Process* pp){
  ...
  list_for_each_safe(p, n, &pidproc->pglist) {
    ...
    page_frame_release(pg->p_addr);
    list_del(p);
    kfree(pg);
  }
  
  page_free((char *) ((u32)pidproc->kstack.esp0 & 0xFFFFF000));

  // Free pages directory
  asm("mov %0, %%eax; mov %%eax, %%cr3"::"m"(pd0));

  page_directory_destroy(pidproc->pd);
